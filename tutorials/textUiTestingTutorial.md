...
1. Add a `runtest.bat` (if you are on Windows) or `runtest.sh` (if you are on a *nix OS) into the folder, containing the script below.<br>
  `runtest.bat`:
   ...
   javac  -cp ..\src -Xlint:none -d ..\bin ..\src\main\java\DukeTest.java
   IF ERRORLEVEL 1 (
       echo ********** BUILD FAILURE **********
       exit /b 1
   )
   ...
   java -classpath ..\bin DukeTest < input.txt > ACTUAL.TXT
   ...
   if ! javac -cp ../src -Xlint:none -d ../bin ../src/main/java/DukeTest.java
   then
       echo "********** BUILD FAILURE **********"
       exit 1
   fi
